...
This repository owns the focused, client-side compatibility-curve application. Released
`wald-inference` is the sole numerical authority. Local code may validate a request, assemble the
focused response, choose a display grid, produce warnings, serialize strict browser JSON, and
render or export the result. It must not implement a Wald formula or add likelihood, support,
power, critical-effect, selection, Type S/M, precision, clinical-threshold, or decision-support
behavior.
...
WebKit verification. Scientific changes require the frozen B01–B03/B08 regression and exact Core
provenance checks. Document every skipped check or warning.
